...
class GoogleTrans(object):
    ...
    def query(self, q, lang_to='zh-CN'):
        q = re.sub(r'''[^\u2E80-\u9FFF \n\t\w_.!'"“”`+-=——,$%^，。？、~@#￥%……|[\]&\\*《》<>「」{}【】()/]''', '', q)
        retry = 3
        while retry > 0:
            try:
...
def TranslateArr(DataWaitToTransArr):
    ...
    while True:
        ...
        originalText, originalLanguageCode, TransResult, targetLanguageCode = GoogleTrans.query(
            DataWaitToTrans[TranslatedPos + 2:EffectivePos])
